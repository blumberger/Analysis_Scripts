...
class XYZ_File(gen_io.DataFileStorage):
	...
	def __init__(self, filepath):
		# If we are going do coord wrapping check if the parameters are there
		#  before any heavy lifting.
		if self.metadata.get("coordinate_wrapping", "") != "":
			wrap_type = self.metadata['coordinate_wrapping']
			self.check_metadata(('cella', 'cellb', 'cellc', 'atoms_per_molecule'))
	...
	def _coord_wrapping_(self):
		"""
		Will call the relevant function to wrap the atoms.

		Wrap Types:
			* "into_cell" => Will wrap molecules back into the cell or as close as possible.
							 Some atoms may still be outside the cell but at least 1 atom in 
		...
		if self.metadata.get("coordinate_wrapping", "") != "":
			...
			if wrap_type in wrap_methods:
				...
				raise SystemExit(f"Coordinate wrapping type '{wrap_type}' not recognised." +
								 "\n\n" + "Please choose from:\n\t*" + 
	...
	def wrap_type_method1(self):
		...
		for istep in range(nstep):
			mols_outside_step_mask = np.all(mols_outside_mask[istep], axis=1)
			...
			xmask_step = x_outside_mask[istep]
			ymask_step = y_outside_mask[istep]
			zmask_step = z_outside_mask[istep]
			step_crds = mol_crds[istep]

			# Loop over molecules outside of the cell
			for imol in mols_outside:
				...
				for mask, dim, cell in ((xmask_step[imol], 0, A),
					...
					if all(mask):
						...
						step_crds[imol, :, dim] -= trans_vec
			
			mol_crds[istep] = step_crds
		...
		for i in keys:
			if i not in self.metadata:
				print("\n\n" + f"Can't find the parameter `{i}`." + "\n"
					  + f"Please set it in the input file as `<var_name>['{i}'] = <data>`"+"\n\n")
				notthere=True
